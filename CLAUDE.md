...
- **Local-path packages**: `doric-themes` is loaded from `/Users/dragon/Documents/Emacs` via `:load-path`, not from a package archive. `gptel-magit` uses `:load-path "~/code/gptel-magit/"` alongside `:straight t`.
...
- **`gptel`** — LLM chat client. Single backend: DeepSeek with thinking enabled (`gptel-make-deepseek "DeepSeek-thinking"` with `:request-params '(:thinking (:type "enabled"))`), API key sourced from `auth-source` via `gptel-api-key-from-auth-source`. Model: `deepseek-v4-pro`. Default chat format: `org-mode`. `gptel-confirm-tool-calls` is nil.
- **`gptel-magit`** — AI-generated commit messages in Magit. Uses Conventional Commits style (`gptel-magit-commit-prompt`). Loaded from local path `~/code/gptel-magit/` (`:load-path` + `:straight t`); the GitHub `roife/gptel-magit` recipe is commented out. Installed on `magit-mode-hook`.
- **`codex-ide-session-mode`** is referenced (not as a package, but as a major mode) in `init-completion.el` — corfu and cape enable in buffers of that mode. There is no `use-package codex-ide` form; `agent-shell` (Codex via ACP) is commented out.
...
| AI | gptel, gptel-magit (local path) |
